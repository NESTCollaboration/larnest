...
namespace larnest 
{
    ...
    LArYieldResult LArNEST::GetRecombinationYields(
        double TotalYields, double ElectronYields,
        double PhotonYields, double energy, double efield
    ) 
    {
    ...
    ) 
    {
        ...
        return GetRecombinationYields(
            NRTotalYields, NRElectronYields, NRPhotonYields,
            energy, efield
        );
    }
    //-------------------------ER Yields-------------------------//
    ...
    {
        return fER.alpha.A + fER.alpha.B / (fER.alpha.C +
    ...
    double LArNEST::GetERElectronYields(
        double energy, double efield, double density
    ) 
    {
    ...
    ) 
    {
        ...
        return GetRecombinationYields(
            ERTotalYields, ERElectronYields, ERPhotonYields,
            energy, efield
        );
    }
    //-------------------------Alpha Yields-------------------------//
    ...
    LArYieldResult LArNEST::GetAlphaYields(
        double energy, double efield, double density
    ) 
    {
    ...
    //---------------------------Lepton LET Yields--------------------------//
    LArYieldResult LArNEST::GetLeptonLETYields(
    ...
    ) 
    {
        ...
        return GetLETRecombinationYields(
            ionization_yields, exciton_yields,
            energy, LET, efield
        );
    }
    //------------------------------LET Yields-----------------------------//
    LArYieldResult LArNEST::GetLETYields(
        double energy, double dx, double efield, double density
    ) 
    {
        LArYieldResult result;
        ...
        return GetLETRecombinationYields(
            ionization_yields, exciton_yields, energy, LET, efield
        );
    }
    ...
    LArYieldResult LArNEST::GetLETRecombinationYields(
        double ionization_yields, double exciton_yields,
    ...
    LArYieldResult LArNEST::GetdEdxRecombinationYields(
        double ionization_yields, double exciton_yields,
    ...
    ) 
    {
        ...
        double exciton_yields = fNexOverNion * ionization_yields;
        return GetdEdxRecombinationYields(
            ionization_yields, exciton_yields, 
            energy, dx, efield
        );
    }
    ...
    ) 
    {
        ...
        {
            Nq = floor(
                RandomGen::GetRandomGen()->rand_zero_trunc_gaussian(Nq_mean, sqrt(Fano * Nq_mean)) +
                0.5);
            if (Nq < 0.) {
                Nq = 0.;
            }
            Nion = double(RandomGen::GetRandomGen()->rand_binomial(Nq, alf));
            if (Nion > Nq) {
                Nion = Nq;
            }
            Nex = Nq - Nion;
        ...
        {
            Nion = floor(RandomGen::GetRandomGen()->rand_zero_trunc_gaussian(
                            Nq_mean * alf, sqrt(Fano * Nq_mean * alf)) +
                        0.5);
            Nex = floor(RandomGen::GetRandomGen()->rand_zero_trunc_gaussian(
    ...
    double LArNEST::GetPhotonTime(
        LArInteraction species, bool exciton, double energy
    ) 
    {
        ...
        double tau1 = RandomGen::GetRandomGen()->rand_zero_trunc_gaussian(6.5, 0.8);  // error from weighted average
        double tau3 = RandomGen::GetRandomGen()->rand_zero_trunc_gaussian(1300, 50);  // ibid.
        double tauR = RandomGen::GetRandomGen()->rand_zero_trunc_gaussian(0.8, 0.2);  // Kubota 1979
        ...
        else 
        {
            ...
            else 
            {
                if (LET < 3.) {
                    if (!exciton) {
                        SingTripRatio = RandomGen::GetRandomGen()->rand_zero_trunc_gaussian(0.5, 0.2);
                    } else if (exciton) {
                        SingTripRatio = RandomGen::GetRandomGen()->rand_zero_trunc_gaussian(0.36, 0.06);
                    }
                } 
                else {
        ...
        double time_ns = tauR * (1.0 / RandomGen::GetRandomGen()->rand_uniform() - 1.);

        if (RandomGen::GetRandomGen()->rand_uniform() <
            SingTripRatio / (1. + SingTripRatio)) {
            time_ns -= tau1 * log(RandomGen::GetRandomGen()->rand_uniform());
        } 
        else {
            time_ns -= tau3 * log(RandomGen::GetRandomGen()->rand_uniform());
        }
        return time_ns;
    }
    ...
    {
        if (state) {
            return RandomGen::GetRandomGen()->rand_zero_trunc_gaussian(9.7, 0.2);
        } else {
            return RandomGen::GetRandomGen()->rand_zero_trunc_gaussian(9.69, 0.22);
        }
    }
    //-------------------------Drift Velocity-------------------------//
    double LArNEST::GetDriftVelocity_Liquid(double Kelvin, double efield) 
    {
        ...
        if (Kelvin < 84. || Kelvin > 140.) 
        {
            ...
            std::cerr << "Using value at closest temp for a drift speed estimate\n";
            if (Kelvin < 84.) {
                Kelvin = 84.;
            } else {
                Kelvin = 140.;
    ...
    */
    ...
    ) 
    {
        ...
        double leftvar =
            RandomGen::GetRandomGen()->rand_zero_trunc_gaussian(yieldFactor, 0.25 * yieldFactor);
        if (leftvar > 1.0) {
            leftvar = 1.0;
        }
        int Nq = int(floor(RandomGen::GetRandomGen()->rand_zero_trunc_gaussian(MeanNq, sigma) + 0.5));
        if (yieldFactor < 1) {
            Nq = RandomGen::GetRandomGen()->rand_binomial(Nq, leftvar);
        }
        ...
        double Nex = RandomGen::GetRandomGen()->rand_binomial(
        ...
        double Nph = Nex + RandomGen::GetRandomGen()->rand_binomial(Nion, recombProb);
        double Ne = Nq - Nph;
    ...
    LArYieldResult LArNEST::LegacyCalculation(
        int pdgcode, double energy, double efield, 
        double density, double track_length
    )
    {
        ...
        return LegacyGetYields(
            energy, efield, yieldFactor, 
            excitationRatio, epsilon, recombProb
        );
    }
